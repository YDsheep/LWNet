...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            s, s1, s2, s3,s4 = model(images,depths)
            ...
            if i % 100 == 0 or i == total_step or i==1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss: {:.4f}, salLoss: {:0.4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss1.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss: {:.4f}, salLoss: {:0.4f}'.
                    format(epoch, opt.epoch, i, total_step, loss1.data, loss.data, loss1.data))
