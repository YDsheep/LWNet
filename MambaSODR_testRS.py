...
parser.add_argument('--test_path',type=str,default='datasets/RGBD_for_test/',help='test dataset path for RS dataset')
...
#load the model
model = WLNet()
model.load_state_dict(torch.load('save/EORSSD/MambaSOD_epoch_best.pth')) # mambaSOD.pth

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
input1 = torch.randn(1, 3, 384, 384)
input1 = input1.to(device)
flops, _ = profile(model.cuda(), inputs=(input1,))
