...
def has_file_allowed_extension(filename, extensions):
    ...
    Returns:
        bool: True if the filename ends with a known RGB extension
    """
...
class CachedImageFolder(DatasetFolder):
    ...
    Args:
        root (string): Root directory path.
        transform (callable, optional): A function/transform that  takes in an PIL RGB
            and returns a transformed version. E.g, ``transforms.RandomCrop``
        target_transform (callable, optional): A function/transform that takes in the
            target and transforms it.
        loader (callable, optional): A function to load an RGB given its path.
     Attributes:
        imgs (list): List of (RGB path, class_index) tuples
    """
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (int): Index
        Returns:
            tuple: (RGB, target) where target is class_index of the target class.
        """
        path, target = self.samples[index]
