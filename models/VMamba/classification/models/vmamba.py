...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        y = y.transpose(dim0=1, dim1=2).contiguous() # (NSI, L, C)
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            def scan_rowcol(
                ...
                xs = torch.stack([xs, xs.flip(dims=[-1])], dim=2) # (NSI, H, 2, D, W)
                if no_einsum:
            ...
            y_row = scan_rowcol(
                ...
                width=True,
            ).view(B, H, 2, -1, W).sum(dim=2).permute(0, 2, 1, 3) # (NSI,C,H,W)
        ...
        if not channel_first:
            y = y.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1) # (NSI, L, C)
        y = out_norm(y)
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        _us = us
        # Bs, Cs = Bs.view(NSI, H, W, 4, -1), Cs.view(NSI, H, W, 4, -1)
        # Bs, Cs = Bs.view(NSI, 4, -1, H, W), Cs.view(NSI, 4, -1, H, W)
...
class SS2Dm0:
    ...
    def forward_corem0(
        ...
        **kwargs,
        ...
        xs = cross_scan_fn(x.view(B, H, W, RD), in_channel_first=False, out_channel_first=False, scans=_scan_mode, force_torch=scan_force_torch) # (NSI, H, W, 4, D)
